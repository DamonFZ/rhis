...
构建一个轻量化、数字化、无纸化的门诊康复管理系统。核心目标是实现客户档案的数字化存储，并强制执行“评估-治疗-对比”的康复 SOP。

## 2.核心约束

- 适用场景：仅限门诊康复业务。
- 功能排除：不包含住院管理、检验检查申请（LIS/PACS）、药房管理、进销存管理、第三方硬件设备对接。
- 存储重点：支持海量静态图片与视频流存储，用于治疗前后对比。
...
- 客户档案管理：
  - 命名规范：客户编号+姓名。
  - 档案树状结构：基本信息、医疗记录、治疗记录（序号+日期）、法务/财务凭证。
- 计次收费系统：
  - 支持套餐购买与余额管理。
  - 治疗后电子回执推送与客户电子签名确认。
...
- 治疗前后影像对比 SOP：
  - 治疗前强制步骤：上传 6 张静态照片（侧立、侧弯、正立、正弯、背立、侧坐）+ 1 段步姿视频（≥20秒）。
  - 治疗后强制步骤：上传 6 张静态照片（其中背坐姿替换侧坐姿）+ 1 段步姿视频（≥20秒）。
...
| record\_type       | tinyint   | 1   | 记录类型：1-治疗前，2-治疗后 |
| treatment\_session | int       | 11  | 第几次治疗            |
...
| PhysicalAssessment | 一对多  | ImagingRecord      | 一次评估可能关联多次影像记录（治疗前/后）  |
